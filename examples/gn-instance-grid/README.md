...
Instance on Points → Realize Instances → Transform — and attaches it as a
`NODES` modifier, following the
...
verts (3 × 3 × 8 = 72), faces = 9 × 6 — and that a `Set Material` node carries
the lime accent onto the evaluated mesh. If Realize Instances is omitted, the
evaluated mesh is empty and the counts fail.
...
It exits non-zero on failure (wrong carrier, topology mismatch, or missing
material). The `blender-smoke` workflow runs the check on Blender 4.5 LTS and 5.1.
