...
def build_signpost_mesh(name, bevel_offset, bevel_segments):
    bm = bmesh.new()
    try:
        wood = []
        boards = []
        ...
        metal.extend(add_square_band(bm, 0.22, half, 0.012, 0.028, METAL_IDX))
...
def setup_bake_image(obj, target_mat, size=BAKE_RES):
    if not obj.data.uv_layers:
        return None, None
    img = bpy.data.images.new("SignpostNrm", size, size, alpha=True, float_buffer=False)
...
def check(skip_decimate):
    ...
    collider_src = build_signpost_mesh(
        "SignpostColSrc", bevel_offset=0.0, bevel_segments=1
    )
